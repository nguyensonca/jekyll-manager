import {
  addFieldToMetadata, removeFieldFromMetadata, updateMetadataFieldKey, updateMetadataFieldValue,
  convertMetadataField, moveMetadataArrayItem, injectDefaultFields
} from '../metadata';
...
describe('Metadata functions:', () => {
  ...
  describe('removeFieldFromMetadata', () => {
    it('should not remove if nameAttr does not exists', () => {
      let actual = removeFieldFromMetadata(emptyState, 'metadata["notexists"]', 'key');
      let expected = {};
      expect(actual).toEqual(expected);
    });
    ...
    it('should remove field', () => {
      let metadata = removeFieldFromMetadata(state, 'metadata["students"][1]', 'name');
  ...
  describe('updateMetadataFieldKey', () => {
    it('should not update if namePrefix does not exists', () => {
      let actual = updateMetadataFieldKey(emptyState, 'metadata', 'notexists', 'no');
      let expected = {};
      expect(actual).toEqual(expected);
    });

    it('should not update if namePrefix does not exists', () => {
      let actual = updateMetadataFieldKey(emptyState, 'metadata["notexists"]', '', '');
      let expected = {};
      expect(actual).toEqual(expected);
    });

    it('should not update if fieldKey does not exists', () => {
      let actual = updateMetadataFieldKey(state, 'metadata', 'layout1', 'layout2');
      let expected = state.metadata;
      expect(actual).toEqual(expected);
    });

    it('should not update if new key already exists', () => {
      let actual = updateMetadataFieldKey(state, 'metadata', 'layout', 'categories');
      let expected = state.metadata;
      expect(actual).toEqual(expected);
    });

    it('should update field key', () => {
      let metadata = updateMetadataFieldKey(state, 'metadata', 'layout', 'layout1');
  ...
  describe('updateMetadataFieldValue', () => {
    ...
    it('should create the key if nameAttr does not exists', () => {
      let actual = updateMetadataFieldValue(emptyState, 'metadata["layout"]', 'post');
      let expected = { layout: 'post' };
      expect(actual).toEqual(expected);
    });

    it('should update an existing key value', () => {
      let metadata = updateMetadataFieldValue(state, 'metadata["layout"]', 'post1');
  ...
  describe('convertMetadataField', () => {
    it('should not convert if nameAttr does not exists', () => {
      let actual = convertMetadataField(state, 'metadata["notexists"]', 'simple');
      expect(actual).toEqual(state.metadata);
    });

    it('should convert simple type to object', () => {
      let metadata = convertMetadataField(state, 'metadata["layout"]', 'object');
      let actual = metadata.layout;
      let expected = { [`New field ${emptyState.new_field_count}`]: '' };
      expect(actual).toEqual(expected);
    });
    ...
    it('should convert array type to object', () => {
      let metadata = convertMetadataField(state, 'metadata["students"]', 'object');
    ...
    it('should convert object type to array', () => {
      let metadata = convertMetadataField(state, 'metadata["students"][1]["name"]', 'array');
      let actual = metadata.students[1].name;
      let expected = [''];
      expect(actual).toEqual(expected);
    });
    ...
    it('should convert twice', () => {
      let metadata = convertMetadataField(state, 'metadata["mentors"][1]', 'object');
      metadata = convertMetadataField({metadata, new_field_count: 1}, 'metadata["mentors"][1]["New field 0"]', 'array');

...
import { getFilenameFromPath, slugify } from '../../utils/helpers';
...
describe('Actions::Collections', () => {
  ...
  it('fetches collections successfully', () => {
    ...
    return store.dispatch(collectionsDuck.fetchCollections()).then(() => {
      // return of async collectionsDuck
  ...
  it("fetches the collection's entries successfully", () => {
    ...
    return store.dispatch(collectionsDuck.fetchCollection('posts')).then(() => {
      expect(store.getActions()).toEqual(expectedActions);
    });
  });
  ...
  it('updates the document successfully', () => {
    ...
    return store
      .dispatch(
        collectionsDuck.putDocument('edit', doc.collection, '', filename)
      )
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  ...
  it('creates PUT_DOCUMENT_FAILURE when updating document failed', () => {
    ...
    return store
      .dispatch(collectionsDuck.putDocument('edit', doc.collection, filename))
      .then(() => {
  ...
  it('creates the document successfully', () => {
    ...
    return store
      .dispatch(collectionsDuck.putDocument('create', new_doc.collection, ''))
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  });
  ...
  it('creates the post with autogenerated filename', () => {
    ...
    return store
      .dispatch(
        collectionsDuck.putDocument('create', new_post_with_date.collection, '')
      )
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  ...
  it('creates the post with autogenerated filename from current date', () => {
    const date = moment().format('YYYY-MM-DD');
    const postName = `${date}-${slugify(new_post_without_date.title)}.md`;
    nock(API)
      .put(`/collections/${new_post_without_date.collection}/${postName}`)
      .reply(200, post);
    ...
    return store
      .dispatch(
        collectionsDuck.putDocument(
          'create',
          new_post_without_date.collection,
          ''
        )
      )
      .then(() => {
  ...
  it('creates the post from a draft', () => {
    nock(API)
      .put(
        `/collections/${new_post_with_date.collection}/${new_post_with_date.path}`
      )
  ...
  it('creates the post in a subdirectory from a draft', () => {
    nock(API)
      .put(
        `/collections/${new_post_with_date.collection}/test/${new_post_with_date.path}`
      )
      .reply(200, post);
  ...
  it('creates VALIDATION_ERROR if the date is not valid.', () => {
    ...
    store.dispatch(collectionsDuck.putDocument('edit', 'posts', doc.id));
    expect(store.getActions()).toEqual(expectedActions);
  });
});

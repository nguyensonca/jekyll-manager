...
describe "templates" do
  ...
  context "index" do
    ...
    it "lists entries in subdirectories" do
      ...
      expect(last_response).to be_ok
      expect(entries.first["name"]).to eq("app.coffee")
      expect(entries.last["name"]).to eq("style.scss")
    end

    it "does not list subdirectories with no relevant entries" do
      get "/templates/assets"
      expect(last_response).to be_ok
      expect(entries.first["name"]).to_not eq("images") # contains static files
      expect(entries.first["name"]).to eq("app.coffee")
    end
  end
